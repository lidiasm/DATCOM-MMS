...
Este primer procedimiento que se emplea para reconocer las etiquetas asignadas a los comentarios del dataset se encuentra ubicado dentro de la categoría anterior de diccionarios. Previo a su uso, en primer lugar cargamos la información contenida en el fichero *subjclueslen1-HLTEMNLP05.tff*, proporcionado en los ejemplos de la asignatura, para luego preprocesarlo y convertilo en un *dataframe*. Esta estructura de datos facilita el acceso a los valores de este diccionario como si de una tabla se tratase, tal y como se muestra en los resultados del siguiente *chunk*. 
...
mpqa_lex <- read.delim("subjclueslen1-HLTEMNLP05.tff", header=FALSE, sep=" ")
# Lista con los nombres de columnas a asignar al dataset
mpqa_lex_cols <- c("type", "len", "word", "word_type", "stemmed", "sentiment")
# Asigna la lista de nombres de columnas al dataset
colnames(mpqa_lex) <- mpqa_lex_cols
# Obtiene el valor de cada par (clave, valor) para almacenarlo en un dataset
for (i in c(1:ncol(mpqa_lex))) {
  mpqa_lex[mpqa_lex_cols[i]] <- substr(mpqa_lex[, i], 
                                 unlist(gregexpr('=', mpqa_lex[, i]))+1, 
                                 nchar(as.character(mpqa_lex[, i])))
}
# Muestra las primeras diez filas del dataset resultante
head(mpqa_lex)
```
